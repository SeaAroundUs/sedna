...
AS WITH distinct_area_taxon AS (
    ...
    JOIN sedna.depth_adjustment_function_eligible_rows er
      ON (d.universal_data_id = er.universal_data_id)
)
...
JOIN sedna.depth_adjustment_function_create_areas dafca
  ON (dat.taxon_key = dafca.taxon_key AND
      dat.allocation_simple_area_id = dafca.allocation_simple_area_id);
